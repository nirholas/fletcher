...
- Both legs are seeded into **Uniswap v4 pools quoted in the underlying**, so a FLOOR at 0.94 NVDA
  ...
  `merge()`.
- **Liquidity is locked for the life of the series.** `collectFees` runs `modifyLiquidity` at zero,
  which returns fees and cannot touch principal, and the only other call site that passes a delta is
  `withdrawPrincipal`, which reverts until 30 days past maturity.
...
Three things bound it, all checked on chain:

- **Depth qualification.** A name must hold in-range Uniswap v3 liquidity across three observations
  spanning eight hours. In-range rather than a balance, because a balance can be raised by a plain
  transfer; sustained rather than instantaneous, because anything read in the calling block can be
  arranged in the calling block with a flash position.
...
- **A leverage ceiling.** The split point may sit at most 98% of spot, which caps TURBO at 50x.

And the lock has an end: `withdrawPrincipal` returns the launcher's position 30 days after maturity.
Locking forever is right for a perpetual token and wrong for a dated one, where liquidity left in
the pool after settlement is principal destroyed on a book nobody will trade again.
...
| Package | What |
|---|---|
| [`contracts`](contracts) | Foundry. `Series`, `FletcherFactory`, `MultiplierAccountant`, `DepthGate`, `FletcherLaunchpad`, `SherwoodSettlementSource`. No owner, no upgrade path, no pause. 92 tests including fuzz invariants, a launchpad suite against Uniswap's real `PoolManager`, regressions for every finding of the September review, and 11 fork tests against live chain 4663. |
...
```bash
pnpm install
pnpm test                              # everything: 161 tests across contracts, SDK, web and keeper
...
```bash
forge test --root contracts            # 81 tests, fork tests skip without an endpoint
