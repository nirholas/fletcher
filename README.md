...
| Package | What |
|---|---|
| [`contracts`](contracts) | Foundry. `Series`, `FletcherFactory`, `MultiplierAccountant`, `DepthGate`, `FletcherLaunchpad`, `SherwoodSettlementSource`. No owner, no upgrade path, no pause. 72 tests including fuzz invariants, a launchpad suite against Uniswap's real `PoolManager`, and 10 fork tests against live chain 4663. |
| [`packages/sdk`](packages/sdk) | `@fletcher/sdk`. Address book, the series arithmetic reimplemented in TypeScript, and a client that batches a series' reads into one multicall. 44 tests, 17 of them asserting parity against a fixture generated by the contracts themselves. |
...
```bash
pnpm install
pnpm test                              # everything: 138 tests across contracts, SDK, web and keeper
...
```bash
forge test --root contracts            # 62 tests, fork tests skip without an endpoint
pnpm --filter @fletcher/sdk test       # 44 tests, 17 parity cases against the contracts
